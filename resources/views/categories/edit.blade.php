        ...
        <form action="{{ route('categories.update', $category) }}" method="POST" class="space-y-6" id="categoryForm">
            ...
            <div>
                ...
                <select name="tipo" id="tipo"
                    ...
                    <option value="ingreso" {{ old('tipo', $category->tipo) == 'ingreso' ? 'selected' : '' }}>Ingreso</option>
                    <option value="gasto" {{ old('tipo', $category->tipo) == 'gasto' ? 'selected' : '' }}>Gasto</option>
                </select>
                @error('tipo')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="icono" id="icono"
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                       value="{{ old('icono', $category->icono) }}"
                       maxlength="20"
